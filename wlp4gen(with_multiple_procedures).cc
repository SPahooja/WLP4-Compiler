...
string standby_procedure = "empty";
string previous_procedure = "empty";
...
void push(int regis){                                   //to push register onto stack
    cout << "sub $30, $30, $4" << endl;
    cout << "sw $" << regis << ", 0($30)" << endl;
}

void pop(int regis){                                   //to pop registers off stack
    cout << "lw $" << regis << ", 0($30)" << endl;
    cout << "add $30, $30, $4" << endl;
}

void load(int regis, int value){                      //to load integer value into register
    cout << "lis $" << regis << endl;
    cout << ".word " << value << endl ;
}

void load(int regis, string value){                      //to load string value into register
    cout << "lis $" << regis << endl;
    cout << ".word " << value << endl;
}

void add(int dest, int reg1, int reg2){                 // to add two registers and store the result in destination register
    cout << "add $" << dest << ", $" << reg1 << ", $" << reg2 << endl;
}

void sub(int dest, int reg1, int reg2){                 // to subtract two registers and store the result in destination register
    cout << "sub $" << dest << ", $" << reg1 << ", $" << reg2 << endl;
}

void multl(int dest, int reg1, int reg2){                 // to multiply two registers and store the lower bit result in destination register
    cout << "mult $" << reg1 << ", $" << reg2 << endl;
    cout << "mflo $" << dest << endl;
}

void multh(int dest, int reg1, int reg2){                 // to multiply two registers and store the higher bit result in destination register
    cout << "mult $" << reg1 << ", $" << reg2 << endl;
    cout << "mfhi $" << dest << endl;
}

void div(int dest, int reg1, int reg2){                 // to divide two registers and store the quotient in destination register
    cout << "div $" << reg1 << ", $" << reg2 << endl;
    cout << "mflo $" << dest << endl;
}

void mod(int dest, int reg1, int reg2){                 // to divide two registers and store the remaiander in destination register
    cout << "div $" << reg1 << ", $" << reg2 << endl;
    cout << "mfhi $" << dest << endl;
}

void jr (int regs){                                     // to jump control flow to a register 
    cout << "jr $" << regs << endl;
}

void jalr (int regs){                                   // to jump control flow(PC) to a register and store current PC to $31
    cout << "jalr $" << regs << endl;
}

void imports(string s){                                 // .import merl files
    cout << ".import " << s << endl;
}

void prologue(){                                        // beginning of assembly code 
    cout << ";SAHIL" << endl;
    cout << ";Prologue" << endl;
    imports("print");
    imports("init");
...
void epilogue(){                                        // end of assmebly code
    cout << ";Epilogue" << endl;
    add(30,29,0);
    jr(31);
}

void init(){    // Initliases the memory allocation data structure and stores either 0 or lenght of array in $2, depending on type of first paramet to wain
    cout << ";Init" << endl;    
...
void lw29(int regis, int offset){                       // load value at offset to $29 into register
    cout << "lw $" << regis << ", " << offset << "($29)" << endl;
}

void lw(int regis, int offset, int addr){                       // load value at offset to address into register
    cout << "lw $" << regis << ", " << offset << "($" << addr << ")" << endl;
}

void sw29(int regis, int offset){                       // set value at offset to $29 from register
    cout << "sw $" << regis << ", " << offset << "($29)" << endl;
}

void sw(int regis, int offset, int addr){                       // set value at offset to address from register
    cout << "sw $" << regis << ", " << offset << "($" << addr << ")" << endl;
}

void beq(int reg1, int reg2, string branch){                // Jump control flow to branch if value of register1 is equal to value of register 2
    cout <<"beq $" << reg1 << ", $" << reg2 << ", " << branch << endl;
}

void bne(int reg1, int reg2, string branch){                // Jump control flow to branch if value of register1 is  NOT equal to value of register 2
    cout <<"bne $" << reg1 << ", $" << reg2 << ", " << branch << endl;
}

void slt(int dest, int reg1, int reg2){                     // if register 1 is less than register 2, set destination register to 1 else 0
    cout << "slt $" << dest << ", $" << reg1 << ", $" << reg2 <<endl; 
}

void sltu(int dest, int reg1, int reg2){                     // if register 1 is less than register 2, set destination register to 1 else 0
    cout << "sltu $" << dest << ", $" << reg1 << ", $" << reg2 <<endl; 
}

string s(int n){                                            // convert an integer into a string HELPER FUNCTION
    stringstream ss;
    ss << n;
    string s;
    ss >> s;
...
void call(string s){                                    // HELPER FUNCTION to call a function 
    push(29);
    push(31);
    load(10,s);
    jalr(10);
...
void comment(string c){                                 // To add comments to assembly code
    cout << ";" << c << endl;
}

void debug(int reg){                                    // HELPER FUNCTION to debug assemly code
    comment("DEBUG");
    sw(1,-4,30);
    sw(31,-8,30);
    load(1,8);
...
void print(string c){                               // helps to write to stdout preventing multuple lines of code to cout 
    cout << c << endl;
}
int final_params = 0;

int offset_of(string v){                            // return the offest of a variable from stack pointer in a procedure  
    if(procedure == "wain" || procedure == "WAIN"){
        return st[procedure].second[v].second;
    }else {
        comment(s(total_params));
        return (st[procedure].second[v].second + (4 * total_params));
    }
    
}
int params(Tree t){                                 // calcualtes the total number of params in a non-WAIN function
    int n;
    if(t.rule == "params paramlist"){
        n = 0;
        n = n + params(t.children[0]);
        return n;
    }else if(t.rule == "params"){
        n = 0;
        return n;
    }else if(t.rule == "paramlist dcl COMMA paramlist"){
        return 1 + params(t.children[2]);
    }else if(t.rule == "paramlist dcl"){
        return 1;
    }
}
void code(Tree t){                     // MAIN CODE GENERATION FUNCTION
    if(t.tokens[0] == "start"){                                           // root node
        ...
        if(t.tokens[2] == "LT"){                // a < b
            ...
            comment("test expr1 == expr2");
            push(6);                //can remove push,pop of 6,7 if not in use
            push(7);                
            if(typeOf(t.children[0]) == "int"){
                slt(6,3,5);
                slt(7,5,3);
        ...
        add(3,11,0);
        sw29(3,offset_of(t.children[1].children[1].tokens[1]));
    }else if(t.rule == "factor NULL"){                                      // assign to NULL
        comment("Assign NULL");
        add(3,11,0);
    }else if(t.rule == "factor STAR factor"){                           // Dereferencing
        comment("Get address of " + t.children[1].children[0].tokens[1]);
        code(t.children[1]);
        lw(3,0,3);
        comment("Set to address of " + t.children[1].children[0].tokens[1]);
    }else if(t.rule == "factor AMP lvalue"){                            // Getting address of a variable
        ...
        code(t.children[1]); 
    }else if(t.rule == "lvalue STAR factor"){                           // Dereferencing
        code(t.children[1]);
    }else if(t.rule == "lvalue ID"){                                    // lvalue ia a variable
        ...
        bne(3,0, s(1));
        add(3,11,0);
    }else if(t.rule == "statement DELETE LBRACK RBRACK expr SEMI"){         //Memory DeAllocation by DELETE
        ...
        delcounter++;
    }else if(t.rule == "procedures procedure procedures"){                  // Support mutiple procedure
        comment("got procedures");
        code(t.children[0]);
        code(t.children[1]);
    }else if(t.rule == "factor ID LPAREN RPAREN"){                          // Calling a function with no parameters
        total_params = params(t);
    ...
    }else if(t.rule == "procedure INT ID LPAREN params RPAREN LBRACE dcls statements RETURN expr SEMI RBRACE"){     // Non-WAIN Function
        previous_procedure = procedure;
        procedure = t.children[1].tokens[1];
        total_params = params(t.children[3]);
        comment("Defining Function: " + t.children[1].tokens[1]);
        print("F" + t.children[1].tokens[1] + ":");
        add(29,30,0);
        comment("Pushing regs");
        push(5);
        push(6);
        push(7);
        comment("Done Pushing");
        code(t.children[6]);
        ...
        comment("Popping regs");
        pop(7);
        pop(6);
        pop(5);
        comment("Done Popping");
        jr(31);
        procedure = previous_procedure;
    }else if(t.rule == "factor ID LPAREN arglist RPAREN"){                  // Calling function with non-zero parameters
        comment("Calling " + t.children[0].tokens[1]);
        push(29);
        push(31);
        push(5);
        push(6);
        push(7);
        standby_procedure = "F" + t.children[0].tokens[1];
        code(t.children[2]);
        pop(7);
        pop(6);
        pop(5);
        pop(31);
        pop(29);
        comment("Done Calling " + t.children[0].tokens[1]);
    }else if(t.rule == "arglist expr"){                                 // Argument for calling function
        code(t.children[0]);
        push(3);
        previous_procedure = procedure;
        procedure = standby_procedure;
        load(10,procedure);
        jalr(10);
        procedure = previous_procedure;
        pop(7);
    }else if(t.rule == "arglist expr COMMA arglist"){                   // Argument list for calling function
        code(t.children[0]);
        push(3);
        code(t.children[2]);
        pop(7);
...
int main(){
    ...
    beq(0,0,"WAIN");
    cout << ";Code" << endl;;
    code(ptree);                // Main code genration function taking the top of parse tree and recursively generating code for selective children trees
